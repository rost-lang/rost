...
let elf32_linux_x86_file
   ...
   *)
    ...
    (* section 16: .note..rost        (segment 5: NOTE)                 *)
    ...
    let note_rost_section_name_fixup = sname ".note.rost" in
  ...
  let note_rost_section_fixup = new_fixup ".shstrtab section" in
  ...
  let shstrtab_section =
    SEQ
      [|
        ...
        DEF (note_rost_section_name_fixup, ZSTRING ".note.rost");
      |]
  in
...
*)
        ...
        (section_header
           ~shstring_table_fixup: shstrtab_section_fixup
           ~shname_string_fixup: note_rost_section_name_fixup
           ~sh_type: SHT_NOTE
           ~sh_flags: []
           ~section_fixup: (Some note_rost_section_fixup)
           ~sh_addralign: 1L
           ~sh_entsize: 0L
           ~sh_link: None);
...
let emit_file
  ...
  let main_fn _ =
    match (start_fixup, rost_start_fixup, sem.Semant.ctxt_main_fn_fixup) with
        (None, _, _)
      | (_, None, _)
      | (_, _, None) -> MARK
      | (Some start_fixup,
