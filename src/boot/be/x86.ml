...
let emit_c_call
  ...
  (* rost calls get task as arg0  *)
  let args =
    if nabi.nabi_convention = CONV_rost
  ...
  in
    ...
    mov                                      (* task->rost_sp = tmp2      *)
      (word_n tmp1 Abi.task_field_rost_sp)
      (c (r tmp2));
    ...
    match ret with
        Il.Mem (Il.RegIn (Il.Hreg base, _), _) when base == esp ->
          ...
          mov (rc esp) (c (word_n tmp2 Abi.task_field_rost_sp));
          mov ret (c (r tmp1));
      ...
      | _ when in_prologue ->
           ...
           * esp *and* ebp via ebp->rost_sp on the other side of
           * the call.
           *)
          mov (rc ebp) (c task_ptr);
          emit (Il.call ret fptr);
          mov (rc esp) (c (word_n (h ebp) Abi.task_field_rost_sp));
          mov (rc ebp) (ro esp);
...
let unwind_glue
  ...
  let exit_jmp_fix = new_fixup "exit jump" in
    ...
    mov (rc edx) (c task_ptr);          (* switch back to rost stack    *)
    mov
      (rc esp)
      (c (edx_n Abi.task_field_rost_sp));
...
let activate_glue (e:Il.emitter) : unit =
  ...
  let binary op dst imm = emit (Il.binary op dst (c dst) (immi imm)) in
    ...
    mov
      (rc esp)
      (c (edx_n Abi.task_field_rost_sp)); (* esp <- task->rost_sp    *)
     ...
     *      doesn't look at task->rost_sp again except when it clobbers it
     ...
     *      task->rost_sp and are about to eat the 5 words off the top of it.
     *
     *
     *      | ...    | <-- where esp will be once we restore + ret, below,
     *      | retpc  |     and where we'd *like* task->rost_sp to wind up.
     *      | ebp    |
     *      | edi    |
     *      | esi    |
     *      | ebx    | <-- current task->rost_sp == current esp
     *
     * 
     *      This is a problem. If we return to "esp <- task->rost_sp" it will
     *      push esp back down by 5 words. This manifests as a rost stack that
     *      grows by 5 words on each yield/reactivate. Not good.
     * 
     *      So what we do here is just adjust task->rost_sp up 5 words as
     *      well, to mirror the movement in esp we're about to perform. That
     *      way the "esp <- task->rost_sp" we 'ret' to below will be a
     *      no-op. Esp won't move, and the task's stack won't grow.
     *)

    binary Il.ADD (edx_n Abi.task_field_rost_sp)
...
let yield_glue (e:Il.emitter) : unit =
  ...
  let mov dst src = emit (Il.umov dst src) in
    ...
    mov
      ...
      (c (edx_n Abi.task_field_rost_sp));    (* esp <- task->rost_sp    *)
    save_callee_saves e;
    mov                                      (* task->rost_sp <- esp    *)
      (edx_n Abi.task_field_rost_sp)
      (ro esp);
    mov
      (rc esp)
...
let objfile_start
  ...
  let push_pos32 = push_pos32 e in
    ...
    let fptr =
      Abi.load_fixup_codeptr e (h eax) rost_start_fixup true indirect_start
    in
