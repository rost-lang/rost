...
let pe_section_header
  ...
  let
    ...
    match id with
      ...
      | SECTION_ID_DEBUG_LINE
      | SECTION_ID_DEBUG_FRAME
      | SECTION_ID_NOTE_ROST -> [ IMAGE_SCN_CNT_INITIALIZED_DATA;
                                  IMAGE_SCN_MEM_READ ]
  in
    SEQ [|
      STRING
        begin
          match id with
            ...
            | SECTION_ID_NOTE_ROST      -> "/86\x00\x00\x00\x00\x00"
        end;
...
let pe_text_section
    ...
    ~(rost_start_fixup:fixup option)
    ~(main_fn_fixup:fixup option)
    ~(text_fixup:fixup)
    ~(crate_code:frag)
    : frag =
  let startup =
    match (start_fixup, rost_start_fixup, main_fn_fixup) with
        (None, _, _)
      | (_, None, _)
      | (_, _, None) -> MARK
      | (Some start_fixup,
         Some rost_start_fixup,
         Some main_fn_fixup) ->
...
let emit_file
  ...
  let note_rost_fixup = new_fixup ".note.rost section" in

  let (start_fixup, rost_start_fixup) =
    if sess.Session.sess_library_mode
    then (None, None)
    else
      (Some (new_fixup "start"),
       Some (Semant.require_native sem REQUIRED_LIB_rostrt "rost_start"))
  in
...
(*
  ...
  in
*)
  let note_rost_header = (pe_section_header
                            ~id: SECTION_ID_NOTE_ROST
                            ~hdr_fixup: note_rost_fixup)
  in
...
(*
  ...
  in
*)
  let note_rost_section =
    def_aligned note_rost_fixup
      (Asm.note_rost_frags crate.node.Ast.crate_meta)
  in
