 ...
 * 
 *)
let note_rost_frags (meta:(Ast.ident * string) array) : frag =
  let desc_fixup = new_fixup ".rost.note metadata" in
  let desc =
    DEF (desc_fixup,
         SEQ [|
...
let read_rost_note (ar:asm_reader) : (Ast.ident * string) array =
  ar.asm_adv_u32 ();
  ar.asm_adv_u32 ();
  assert ((ar.asm_get_u32 ()) = 0);
  let rost_name = ar.asm_get_zstr_padded 8 in
    assert (rost_name = "rost");
