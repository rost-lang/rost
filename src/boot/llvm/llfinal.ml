...
let finalize_module
  ...
  (*
   * Count the number of Rost functions and the number of C functions by
   * simply (and crudely) testing whether each function in the module begins
   * with "_rost_".
   *)

  let (rost_fn_count, c_fn_count) =
    let count (rost_fn_count, c_fn_count) fn =
      ...
      in
      if begins_with "_rost_" (Llvm.value_name fn) then
        (rost_fn_count + 1, c_fn_count)
      else
        (rost_fn_count, c_fn_count + 1)
    in
  ...
  let crate_val =
    ...
    Llvm.const_struct llctx [|
      ...
      Llvm.const_int i32 rost_fn_count; (* int n_rost_syms *)
  ...
  let rost_main_fn =
    match Llvm.lookup_function "_rost_main" llmod with
        None -> raise (Failure "no main function found")
      | Some fn -> fn
  in
  let rost_start = abi.Llabi.rost_start in
  let rost_start_args = [| rost_main_fn; crate_ptr; argc; argv |] in
    ignore (Llvm.build_call
              rost_start rost_start_args "start_rost" main_builder);
