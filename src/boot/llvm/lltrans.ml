...
let trans_crate
  ...
  let (crate_ptr:Llvm.llvalue) =
    Llvm.declare_global abi.Llabi.crate_ty "rost_crate" llmod
  in
    ...
    in
      match item with
          Ast.MOD_ITEM_fn _ ->
            ...
            let llfn = Llvm.declare_function ("_rost_" ^ name) llty llmod in
            let meta =
              md_node
                [|
  ...
  let trans_fn
     ...
     * Rost), and store them in the slot-to-llvalue mapping. *)
  ...
  let exit_task_glue =
    ...
    let llfn = Llvm.declare_function "rost_exit_task_glue" llty llmod in
