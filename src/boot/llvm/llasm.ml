...
let x86_glue
  ...
  let restore_callee_saves =
    ["popl  %ebx";
     ...
     "popl  %ebp";]
  in
  let load_esp_from_rost_sp    = ["movl  12(%edx), %esp"] in
  let load_esp_from_runtime_sp = ["movl   8(%edx), %esp"] in
  let store_esp_to_rost_sp     = ["movl  %esp, 12(%edx)"] in
  ...
  let glue =
    [
      ("rost_activate_glue",
       String.concat "\n\t"
         (["movl  4(%esp), %edx    # edx = rost_task"]
          @ save_callee_saves
          @ store_esp_to_runtime_sp
          @ load_esp_from_rost_sp
            (* 
      ...
      ("rost_yield_glue",
       String.concat "\n\t"
         ...
         (["movl  0(%esp), %edx    # edx = rost_task"]
          @ load_esp_from_rost_sp
          @ save_callee_saves
          @ store_esp_to_rost_sp
    ...
    @ list_init n_upcall_glues
      begin
        fun i ->
          ...
          ((Printf.sprintf "rost_upcall_%d" i),
           String.concat "\n\t"
             (save_callee_saves
              @ ["movl  %esp, %ebp     # ebp = rost_sp";
                 "movl  20(%esp), %edx # edx = rost_task"]
              @ store_esp_to_rost_sp
              @ load_esp_from_runtime_sp
              @ [Printf.sprintf
                   "subl  $%d, %%esp   # esp -= args" ((i+1)*4);
                 "andl  $~0xf, %esp    # align esp down";
                 "movl  %edx, (%esp)   # arg[0] = rost_task "]
  ...
  let decl_upcall n =
    ...
    in
    let name = Printf.sprintf "rost_upcall_%d" n in
      decl_cdecl_fn name word_ty args_ty
  in
    {
      asm_activate_glue = decl_glue "rost_activate_glue";
      asm_yield_glue = decl_glue "rost_yield_glue";
