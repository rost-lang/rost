(*
 * LLVM integration with the Rost runtime.
 *)
...
let declare_abi (llctx:Llvm.llcontext) (llmod:Llvm.llmodule) : abi =
  ...
  in
  ignore (Llvm.define_type_name "rost_crate" crate_ty llmod);
  ...
  let task_ty =
    ...
    Llvm.struct_type llctx [|
      ...
      Llvm.pointer_type i32;  (* uintptr_t rost_sp *)
      Llvm.pointer_type i32;  (* rost_rt *rt *)
      Llvm.pointer_type i32   (* rost_crate_cache *cache *)
    |]
  in
  ignore (Llvm.define_type_name "rost_task" task_ty llmod);
  ...
  in
  {
    ...
    task_ty = task_ty;
    word_ty = i32;
    rost_start = Llvm.declare_function "rost_start" rost_start_ty llmod
  }
;;
