 ...
 *     support major components of the rost runtime (reflection,
...
let dw_tag_to_string (tag:dw_tag) : string =
  ...
  | DW_TAG_rost_meta -> "DW_TAG_rost_meta"
...
type dw_at =
  ...
  | DW_AT_recursive
  | DW_AT_lo_user
  | DW_AT_rost_type_code
  | DW_AT_rost_type_param_index
  | DW_AT_rost_iterator
  | DW_AT_rost_native_type_id
  | DW_AT_hi_user
;;
...
let dw_at_to_int (a:dw_at) : int =
  match a with
    ...
    | DW_AT_rost_type_code -> 0x2300
    | DW_AT_rost_type_param_index -> 0x2301
    | DW_AT_rost_iterator -> 0x2302
    | DW_AT_rost_native_type_id -> 0x2303
...
let dw_at_of_int (i:int) : dw_at =
  match i with
    ...
    | 0x2300 -> DW_AT_rost_type_code
    | 0x2301 -> DW_AT_rost_type_param_index
    | 0x2302 -> DW_AT_rost_iterator
    | 0x2303 -> DW_AT_rost_native_type_id
...
let dw_at_to_string (a:dw_at) : string =
  match a with
    ...
    | DW_AT_rost_type_code -> "DW_AT_rost_type_code"
    | DW_AT_rost_type_param_index -> "DW_AT_rost_type_param_index"
    | DW_AT_rost_iterator -> "DW_AT_rost_iterator"
    | DW_AT_rost_native_type_id -> "DW_AT_native_type_id"
...
let dw_rost_type_to_int (pt:dw_rost_type) : int =
  match pt with
      DW_ROST_type_param -> 0x1
    | DW_ROST_nil -> 0x2
    | DW_ROST_vec -> 0x3
    | DW_ROST_chan -> 0x4
    | DW_ROST_port -> 0x5
...
let dw_rost_type_of_int (i:int) : dw_rost_type =
  match i with
      0x1 -> DW_ROST_type_param
    | 0x2 -> DW_ROST_nil
    | 0x3 -> DW_ROST_vec
    | 0x4 -> DW_ROST_chan
    | 0x5 -> DW_ROST_port
    ...
    | 0x9 -> DW_ROST_type
    | 0xa -> DW_ROST_native
    | _ -> bug () "bad DWARF rost-pointer-type code: %d" i
;;
...
let (abbrev_meta:abbrev) =
  (DW_TAG_rost_meta, DW_CHILDREN_no,
   [|
     (DW_AT_name, DW_FORM_string);
...
let (abbrev_unspecified_structure_type:abbrev) =
  (DW_TAG_structure_type, DW_CHILDREN_no,
   [|
     (DW_AT_rost_type_code, DW_FORM_data1);
     (DW_AT_declaration, DW_FORM_flag);
   |])
;;
...
let (abbrev_native_pointer_type:abbrev) =
  (DW_TAG_pointer_type, DW_CHILDREN_no,
   [|
     (DW_AT_rost_type_code, DW_FORM_data1);
     (DW_AT_rost_native_type_id, DW_FORM_data4)
   |])
;;

let (abbrev_rost_type_param:abbrev) =
  (DW_TAG_pointer_type, DW_CHILDREN_no,
   [|
     (DW_AT_rost_type_code, DW_FORM_data1);
     (DW_AT_rost_type_param_index, DW_FORM_data4);
     (DW_AT_mutable, DW_FORM_flag);
     (DW_AT_pure, DW_FORM_flag);
   |])
;;

let (abbrev_rost_type_param_decl:abbrev) =
  (DW_TAG_formal_parameter, DW_CHILDREN_no,
   [|
     (DW_AT_rost_type_code, DW_FORM_data1);
     (DW_AT_name, DW_FORM_string);
...
let (abbrev_subroutine_type:abbrev) =
    (DW_TAG_subroutine_type, DW_CHILDREN_yes,
     [|
       ...
       (DW_AT_mutable, DW_FORM_flag);
       (DW_AT_pure, DW_FORM_flag);
       (DW_AT_rost_iterator, DW_FORM_flag);
     |])
;;
...
let dwarf_visitor
  ...
  let type_param_die (p:(ty_param_idx * Ast.effect)) =
    let (idx, eff) = p in
      SEQ [|
        uleb (get_abbrev_code abbrev_rost_type_param);
        (* DW_AT_rost_type_code: DW_FORM_data1 *)
        BYTE (dw_rost_type_to_int DW_ROST_type_param);
        (* DW_AT_rost_type_param_index: DW_FORM_data4 *)
  ...
  and ref_type_die
    ...
    then Hashtbl.find emitted_types ty
      ...
      let rost_type_param (p:(ty_param_idx * Ast.effect)) =
        let fix = new_fixup "rost-type-param DIE" in
        let die = DEF (fix, type_param_die p) in
          emit_die die;
          ref_addr_for_fix fix
      in

      let unspecified_ptr_with_ref rost_ty ref_addr =
      ...
      let unspecified_ptr_with_ref_ty rost_ty ty =
        unspecified_ptr_with_ref rost_ty (ref_type_die ty)
      in

      let unspecified_ptr_with_ref_slot rost_ty slot =
        unspecified_ptr_with_ref rost_ty (ref_slot_die slot)
      in

      let unspecified_ptr rost_ty =
        unspecified_ptr_with_ref rost_ty (unspecified_anon_struct ())
      in
      ...
      let native_ptr_type oid =
        ...
        let die =
          DEF (fix, SEQ [|
                 ...
                 (* DW_AT_rost_type_code: DW_FORM_data1 *)
                 BYTE (dw_rost_type_to_int DW_ROST_native);
                 (* DW_AT_rost_native_type_id: DW_FORM_data4 *)
      ...
      in
        ...
        match ty with
            Ast.TY_nil -> unspecified_struct DW_ROST_nil
          ...
          | Ast.TY_vec s -> unspecified_ptr_with_ref_slot DW_ROST_vec s
          | Ast.TY_chan t -> unspecified_ptr_with_ref_ty DW_ROST_chan t
          | Ast.TY_port t -> unspecified_ptr_with_ref_ty DW_ROST_port t
          | Ast.TY_task -> unspecified_ptr DW_ROST_task
          | Ast.TY_fn fn -> fn_type fn
          | Ast.TY_tag _ -> unspecified_ptr DW_ROST_tag
          | Ast.TY_iso _ -> unspecified_ptr DW_ROST_iso
          | Ast.TY_type -> unspecified_ptr DW_ROST_type
          | Ast.TY_native i -> native_ptr_type i
          | Ast.TY_param p -> rost_type_param p
  ...
  let type_param_decl_die (p:(Ast.ident * (ty_param_idx * Ast.effect))) =
    let (ident, (idx, eff)) = p in
      SEQ [|
        uleb (get_abbrev_code abbrev_rost_type_param_decl);
        (* DW_AT_rost_type_code: DW_FORM_data1 *)
        BYTE (dw_rost_type_to_int DW_ROST_type_param);
        (* DW_AT_name:  DW_FORM_string *)
...
let rec extract_mod_items
  ...
  let get_type_param die =
    let idx = get_num die DW_AT_rost_type_param_index in
    let e = get_effect die in
      (idx, e)
  in

  let get_native_id die =
    get_num die DW_AT_rost_native_type_id
  in
  ...
  let is_rost_type die t =
    match atab_search die.die_attrs DW_AT_rost_type_code with
        Some (_, DATA_num n) -> (dw_rost_type_of_int n) = t
      | _ -> false
  in
  ...
  let rec get_ty die : Ast.ty =
      match die.die_tag with
          ...
          DW_TAG_structure_type
            when is_rost_type die DW_ROST_nil ->
              Ast.TY_nil

        | DW_TAG_pointer_type
            when is_rost_type die DW_ROST_task ->
            Ast.TY_task

        | DW_TAG_pointer_type
            when is_rost_type die DW_ROST_type ->
            Ast.TY_type

        | DW_TAG_pointer_type
            when is_rost_type die DW_ROST_port ->
            Ast.TY_port (get_referenced_ty die)

        | DW_TAG_pointer_type
            when is_rost_type die DW_ROST_chan ->
            Ast.TY_chan (get_referenced_ty die)

        | DW_TAG_pointer_type
            when is_rost_type die DW_ROST_vec ->
            Ast.TY_vec (get_referenced_slot die)

        | DW_TAG_pointer_type
            when is_rost_type die DW_ROST_type_param ->
            Ast.TY_param (get_type_param die)

        | DW_TAG_pointer_type
            when is_rost_type die DW_ROST_native ->
  ...
  and get_ty_fn die =
    ...
    let iter = get_flag die DW_AT_rost_iterator in
    let tsig =
        ...
        begin
          fun child ->
            match child.die_tag with
                DW_TAG_formal_parameter ->
                  if (is_rost_type child DW_ROST_type_param)
