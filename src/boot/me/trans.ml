...
let trans_visitor
  ...
  and trans_glue_frame_entry
    ...
    let framesz = SIZE_fixup_mem_sz spill in
      ...
      abi.Abi.abi_emit_fn_prologue (emitter())
        framesz callsz nabi_rost (upcall_fixup "upcall_grow_task");
  ...
  and upcall_fixup (name:string) : fixup =
    Semant.require_native cx REQUIRED_LIB_rostrt name;
  ...
  and trans_upcall
    ...
    abi.Abi.abi_emit_native_call (emitter())
      ret nabi_rost (upcall_fixup name) args;
  ...
  and trans_void_upcall
    ...
    abi.Abi.abi_emit_native_void_call (emitter())
      nabi_rost (upcall_fixup name) args;
  ...
  let trans_frame_entry (fnid:node_id) : unit =
    ...
    let callsz = get_callsz cx fnid in
      ...
      abi.Abi.abi_emit_fn_prologue
        (emitter()) framesz callsz nabi_rost
        ...
        match ilib with
            REQUIRED_LIB_rost ls ->
              begin
                ...
                in
                let rost_sym_num =
                  htab_search_or_add cx.ctxt_required_rost_sym_num fnid
                    (fun _ -> Hashtbl.length cx.ctxt_required_rost_sym_num)
                in
                ...
                in
                  trans_upcall "upcall_require_rost_sym" f
                    [| Il.Cell (curr_crate_ptr());
                       imm (Int64.of_int lib_num);
                       imm (Int64.of_int c_sym_num);
                       imm (Int64.of_int rost_sym_num);
                       libstr;
                       relpath |];
          ...
          | _ -> bug ()
              "Trans.required_rost_fn on unexpected form of require library"
      end;
      emit Il.Leave;
      match ilib with
          REQUIRED_LIB_rost _ ->
      ...
      (cx.ctxt_crate_fixup,
       Asm.DEF
         (cx.ctxt_crate_fixup,
          Asm.SEQ [|
            (* 
             * NB: this must match the rost_crate structure
             * in the rost runtime library.
             *)
            ...
            tab_sz cx.ctxt_required_rost_sym_num;
    ...
    in
      ...
      emit_aux_global_glue cx GLUE_unwind
        ...
        (fun e -> abi.Abi.abi_unwind
           e nabi_rost (upcall_fixup "upcall_exit"));
      ...
      provide_existing_native cx SEG_data "rost_crate" cx.ctxt_crate_fixup;
