...
static stk_seg*
new_stk(rost_dom *dom, size_t minsz)
{
...
static void
del_stk(rost_dom *dom, stk_seg *stk)
{
...
rost_task::rost_task(rost_dom *dom, rost_task *spawner) :
    stk(new_stk(dom, 0)),
    runtime_sp(0),
    rost_sp(stk->limit),
    gc_alloc_chain(0),
...
rost_task::~rost_task()
{
    dom->log(rost_log::MEM|rost_log::TASK,
             "~rost_task 0x%" PRIxPTR ", refcnt=%d",
    ...
    /*
      ...
      dom->log(rost_log::MEM|rost_log::TASK,
      "~rost_task, frame fp=0x%" PRIxPTR ", glue_fns=0x%" PRIxPTR,
      fp, glue_fns);
      if (glue_fns) {
      dom->log(rost_log::MEM|rost_log::TASK,
               "~rost_task, mark_glue=0x%" PRIxPTR,
               glue_fns->mark_glue);
      dom->log(rost_log::MEM|rost_log::TASK,
               "~rost_task, drop_glue=0x%" PRIxPTR,
               glue_fns->drop_glue);
      dom->log(rost_log::MEM|rost_log::TASK,
               "~rost_task, reloc_glue=0x%" PRIxPTR,
               glue_fns->reloc_glue);
      }
      }
    */
...
void
rost_task::start(uintptr_t exit_task_glue,
                 ...
                 size_t callsz)
{
    ...
    rost_sp -= sizeof(uintptr_t);
    rost_sp = align_down(rost_sp);
    ...
    uintptr_t *spp = (uintptr_t *)rost_sp;
...
rost_task::grow(size_t n_frame_bytes)
{
    ...
    dom->log(rost_log::MEM|rost_log::TASK, "upcall_grow_task growing stk 0x%"
    ...
    nstk->limit = new_top;
    this->stk = nstk;
    this->rost_sp = new_top - rost_sp_disp;

    dom->log(rost_log::MEM|rost_log::TASK, "processing relocations");
    ...
    for (uintptr_t* p = (uintptr_t*)(new_top - n_copy);
        ...
        if (old_bottom <= *p && *p < old_top) {
            //dom->log(rost_log::MEM, "relocating pointer 0x%" PRIxPTR
...
void
rost_task::run_after_return(size_t nargs, uintptr_t glue)
{
    ...
    dom->log(rost_log::TASK|rost_log::MEM,
             "run_after_return: overwriting retpc=0x%" PRIxPTR
             " @ runtime_sp=0x%" PRIxPTR
             " with glue=0x%" PRIxPTR,
             *retpc, sp, glue);

    // Move the current return address (which points into rost code)
    // onto the rost stack and pretend we just called into the glue.
    push_onto_thread_stack(rost_sp, *retpc);
    *retpc = glue;
}

void
rost_task::run_on_resume(uintptr_t glue)
{
    // This is only safe to call if we're suspended.
    check_suspended();

    // Inject glue as resume address in the suspended frame.
    uintptr_t* rsp = (uintptr_t*) rost_sp;
    rsp += n_callee_saves;
    dom->log(rost_log::TASK|rost_log::MEM,
...
void
rost_task::yield(size_t nargs)
{
    dom->log(rost_log::TASK,
...
rost_task::kill() {
    ...
    dom->log(rost_log::TASK, "killing task 0x%" PRIxPTR, this);
...
void
rost_task::fail(size_t nargs) {
    // See note in ::kill() regarding who should call this.
    dom->log(rost_log::TASK, "task 0x%" PRIxPTR " failing", this);
    // Unblock the task so it can unwind.
    unblock();
    if (this == dom->root_task)
        dom->fail();
...
void
rost_task::notify_waiting_tasks()
{
    while (waiting_tasks.length() > 0) {
        rost_task *t = waiting_tasks.pop()->receiver;
        if (!t->dead())
            t->wakeup(this);
    }
}
...
rost_task::get_fp() {
    ...
    // the task stack.
    return get_callee_save_fp((uintptr_t*)rost_sp);
}

uintptr_t
rost_task::get_previous_fp(uintptr_t fp) {
...
frame_glue_fns*
rost_task::get_frame_glue_fns(uintptr_t fp) {
    fp -= sizeof(uintptr_t);
...
bool
rost_task::blocked_on(rost_cond *on)
{
...
void
rost_task::transition(ptr_vec<rost_task> *src, ptr_vec<rost_task> *dst)
{
    I(dom, state == src);
    dom->log(rost_log::TASK,
...
void
rost_task::wakeup(rost_cond *from)
{
...
rost_crate_cache *
rost_task::get_crate_cache(rost_crate const *curr_crate)
{
    if (cache && cache->crate != curr_crate) {
        dom->log(rost_log::TASK, "switching task crate-cache to crate 0x%"
                 PRIxPTR, curr_crate);
        cache->deref();
        cache = NULL;
    }
    ...
    if (!cache) {
        dom->log(rost_log::TASK, "fetching cache for current crate");
