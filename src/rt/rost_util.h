...
template <typename T>
ptr_vec<T>::ptr_vec(rost_dom *dom) :
    dom(dom),
    alloc(INIT_SIZE),
    fill(0),
...
// Vectors (rost-user-code level).

struct
rost_vec : public rc_base<rost_vec>
{
    size_t alloc;
    size_t fill;
    uint8_t data[];
    rost_vec(rost_dom *dom, size_t alloc, size_t fill, uint8_t const *d) :
        alloc(alloc),
        fill(fill)
    {
        if (d || fill) {
...
// Rost types vec and str look identical from our perspective.
typedef rost_vec rost_str;
