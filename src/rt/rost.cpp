...
rost_srv::~rost_srv()
{
    if (live_allocs != 0) {
        ...
        snprintf(msg, sizeof(msg),
                 "leaked memory in rost main loop (%" PRIuPTR " objects)",
                 live_allocs);
#ifdef TRACK_ALLOCATIONS
...
void *
rost_srv::realloc(void *p, size_t bytes)
{
    if (!p) {
        live_allocs++;
    }
...
void
rost_srv::fatal(char const *expr, char const *file, size_t line)
{
    char buf[1024];
...
rost_main_loop(rost_dom *dom)
{
    ...
    rost_timer timer(*dom);

    int rval;
    rost_task *task;

    dom->log(rost_log::DOM,
    ...
    while ((task = dom->sched()) != NULL) {
        ...
        I(dom, task->rost_sp >= (uintptr_t) &task->stk->data[0]);
        I(dom, task->rost_sp < task->stk->limit);

        dom->reap_dead_tasks();
    }

    dom->log(rost_log::DOM, "finished main-loop (dom.rval = %d)", dom->rval);
    rval = dom->rval;
...
command_line_args
{
    ...
    // vec[str] passed to rost_task::start.
    rost_vec *args;

    command_line_args(rost_dom &dom,
                      int sys_argc,
                      char **sys_argv)
        : dom(dom),
          argc(sys_argc),
          ...
          args(NULL)
    {
        ...
        LocalFree(wargv);
#endif
        size_t vec_fill = sizeof(rost_str *) * argc;
        size_t vec_alloc = next_power_of_two(sizeof(rost_vec) + vec_fill);
        void *mem = dom.malloc(vec_alloc);
        args = new (mem) rost_vec(&dom, vec_alloc, 0, NULL);
        rost_str **strs = (rost_str**) &args->data[0];
        for (int i = 0; i < argc; ++i) {
            size_t str_fill = strlen(argv[i]) + 1;
            size_t str_alloc = next_power_of_two(sizeof(rost_str) + str_fill);
            mem = dom.malloc(str_alloc);
            strs[i] = new (mem) rost_str(&dom, str_alloc, str_fill,
...
extern "C" CDECL int
rost_start(uintptr_t main_fn, rost_crate const *crate, int argc, char **argv)
{
    int ret;
    {
        rost_srv srv;
        rost_dom dom(&srv, crate);
        command_line_args args(dom, argc, argv);

        dom.log(rost_log::DOM, "startup: %d args", args.argc);
        for (int i = 0; i < args.argc; ++i)
            dom.log(rost_log::DOM,
                    "startup: arg[%d] = '%s'", i, args.argv[i]);

        if (dom._log.is_tracing(rost_log::DWARF)) {
