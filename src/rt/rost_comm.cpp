...
template class ptr_vec<rost_token>;
template class ptr_vec<rost_alarm>;
template class ptr_vec<rost_chan>;

rost_alarm::rost_alarm(rost_task *receiver) :
    receiver(receiver)
{
}
...
circ_buf::circ_buf(rost_dom *dom, size_t unit_sz) :
    dom(dom),
...
circ_buf::push(void *src)
{
    ...
    dom->log(rost_log::MEM|rost_log::COMM, "pushed data at index %d", i);
    unread += unit_sz;
}
...
circ_buf::shift(void *dst)
{
    ...
    dom->log(rost_log::MEM|rost_log::COMM, "shifted data from index %d", i);
    unread -= unit_sz;
    next += unit_sz;
    I(dom, next <= alloc);
    if (next == alloc)
...
rost_port::rost_port(rost_task *task, size_t unit_sz) :
    task(task),
    unit_sz(unit_sz),
    writers(task->dom),
    chans(task->dom)
{
    ...
    dom->log(rost_log::MEM|rost_log::COMM,
             "new rost_port(task=0x%" PRIxPTR ", unit_sz=%d) -> port=0x%"
...
{
    rost_dom *dom = task->dom;
    dom->log(rost_log::COMM|rost_log::MEM,
             "~rost_port 0x%" PRIxPTR,
             (uintptr_t)this);
...
rost_token::rost_token(rost_chan *chan) :
    chan(chan),
    idx(0),
    submitted(false)
{
...
rost_token::~rost_token()
{
}

bool
rost_token::pending() const
{
    return submitted;
}
