...
// all*, and without the timer thread we're unable to otherwise preempt rost
...
timer_loop(void *ptr)
{
    // We were handed the rost_timer that owns us.
    rost_timer *timer = (rost_timer *)ptr;
    rost_dom &dom = timer->dom;
    dom.log(rost_log::TIMER, "in timer 0x%" PRIxPTR, (uintptr_t)timer);
...
rost_timer::rost_timer(rost_dom &dom) : dom(dom), exit_flag(0)
{
    dom.log(rost_log::TIMER, "creating timer for domain 0x%" PRIxPTR, &dom);
#if defined(__WIN32__)
